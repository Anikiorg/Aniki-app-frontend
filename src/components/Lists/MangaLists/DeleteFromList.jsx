...
function DeleteFromList(props) {
    ...
    const handleDelete = () => {
            axios.put(`${process.env.REACT_APP_API_URL}/api/users/${user.userName}/mangaremove`, {mangaId: props.mangaId, case: props.case}, { headers: { Authorization: `Bearer ${storedToken}` }})
            .then(()=> {
                props.showList()
            })
            .catch((err)=> err)
...
return (
    ...
    <button className="btn btn-ghost bottom-button" onClick={handleDelete}>
